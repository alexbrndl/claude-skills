#!/usr/bin/env node
// Adapted from huashu-design (MIT) — see CREDITS.md. Records an HTML animation to MP4 via Playwright + ffmpeg.
/**
...
(async () => {
  ...
  // should also honor this signal (see references/pitfalls.md, "No looping while recording").
  ...
  if (hasReady) {
    // Second line of defense: actively reset the animation time to zero — for
    // HTML that does not strictly follow the starter tick template (e.g. lastTick
    // initialized with performance.now(), which folds font-load time into the
    // first frame's dt). See references/pitfalls.md (pure render principle).
    ...
    // references/pitfalls.md). Loud warning so user knows to fix the HTML.
    console.log('');
    ...
    console.log(`     own Stage, see references/pitfalls.md (recording-start offset) for the pattern.`);
    console.log('');
  }
